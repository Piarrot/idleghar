...
public class MockEmailsProvider : IEmailsProvider
{
    ...
    {
        this.EmailTemplates.Add(EmailTemplateNames.VALIDATION_CODE, new EmailTemplate
        {
            Message = "Code: {code}",
    ...
    public async Task<EmailTemplate> GetTemplate(string templateName)
    {
        ...
        this.EmailTemplates.TryGetValue(EmailTemplateNames.VALIDATION_CODE, out template);
        await Task.Yield();
        if (template != null)
        {
            return template;
